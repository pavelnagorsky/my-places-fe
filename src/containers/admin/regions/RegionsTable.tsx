...
import RegionsTableHead from "@/containers/admin/regions/RegionsTableHead";

const RegionsTable = ({ searchText }: ITableProps) => {
  const { i18n } = useTranslation();
  const [items, setItems] = useState<IRegion[]>([]);
  ...
  function handleClick(item: IRegion) {
    router.push(`/administration/regions/${item.id}`);
  }

...
const googlePlacesAutocompleteService = {
  ...
  findPlaceByPlaceId: (placeId: string) => {
    const url = `https://maps.googleapis.com/maps/api/geocode/json?place_id=${placeId}&key=${Environment.googleMapsKey}`;
    return axios.get<google.maps.GeocoderResponse>(url);
  },

  getLocationTitle: (coordinates: ILatLngCoordinate, language: string) => {
    const url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${coordinates.lat},${coordinates.lng}&language=${language}&key=${Environment.googleMapsKey}`;

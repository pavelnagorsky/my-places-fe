...
export function usePolygons({ address }: IUsePolygonsProps) {
  ...
  useEffect(() => {
    ...
    openStreetMapService
      .getPolygonData(address.structured_formatting.main_text)
      .then(({ data }) => {
        // filter data to include only polygons and multi-polygons
        const filterGeoJsonType = data.filter(
          (d) =>
            d.geojson.type === POLYGON_TYPES.MULTI_POLYGON ||
            d.geojson.type === POLYGON_TYPES.POLYGON
        );
